...
fn test_protocol_switch() {
    for (req, deny, accept) in vec![
        ...
        let setup = || {
            ...
            // finish the request before that kicks in
            for (_, connection) in &mut p.conn {
                assert_eq!(connection.get_states()[&Role::Client], h11::State::SendBody);
            }
            p.send(
                Role::Client,
                vec![
        ...
        for (_, connection) in &mut p.conn {
            assert_eq!(
                connection.get_states(),
                HashMap::from([
                    (Role::Client, h11::State::SwitchedProtocol),
                    (Role::Server, h11::State::SwitchedProtocol)
                ])
            );
...
fn test_close_simple() {
        ...
        vec![(Role::Client, Role::Server), (Role::Server, Role::Client)]
    {
        let setup = || {
            ...
            for (_, connection) in &mut p.conn {
                assert_eq!(
                    connection.get_states(),
                    HashMap::from([
                        (who_shot_first, h11::State::Closed),
                        (who_shot_second, h11::State::MustClose)
                    ])
                );
            }
            return p;
        ...
        .unwrap();
        for (_, connection) in &mut p.conn {
            assert_eq!(connection.get_our_state(), h11::State::Closed);
            assert_eq!(connection.get_their_state(), h11::State::Closed);
        }
        // But trying to receive new data on a closed connection is a
        // RuntimeError (not ProtocolError, because the problem here isn't
        // violation of HTTP, it's violation of physics)
...
fn test_pipelined_close() {
    ...
    assert_eq!(c.get_their_state(), h11::State::Closed);
    assert_eq!(c.get_our_state(), h11::State::Closed);
}
...
fn test_errors() {
    ...
    for role in vec![Role::Client, Role::Server] {
        ...
        // Now any attempt to receive continues to raise
        assert_eq!(c.get_their_state(), h11::State::Error);
        assert_ne!(c.get_our_state(), h11::State::Error);
        assert!(match c.next_event().unwrap_err() {
            ProtocolError::RemoteProtocolError(_) => true,
            _ => false,
        });
    ...
    for role in vec![Role::Client, Role::Server] {
        ...
        assert_eq!(c.get_our_state(), h11::State::Error);
        assert_ne!(c.get_their_state(), h11::State::Error);
        // Now 'good' is not so good
