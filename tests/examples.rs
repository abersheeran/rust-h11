...
fn test_100_continue() {
    ...
    assert!(!p.conn[Role::Client].they_are_waiting_for_100_continue);
    assert!(p.conn[Role::Server].they_are_waiting_for_100_continue);

    // Disabled by 100 Continue
    p = ConnectionPair::new();
    p.send(
        Role::Server,
        vec![InformationalResponse::new(100, vec![]), EndOfMessage], // type: ignore[arg-type]
        // ...but oh look, it arrived anyway
        vec![
            Response::new(204, vec![("connection", "close")]),
            EndOfMessage,
        ],
    );
    for conn in p.conns {
        assert!(!conn.client_is_waiting_for_100_continue);
        assert!(!conn.they_are_waiting_for_100_continue);
    }
    ...
    p.send(
        Role::Server,
        Response::new(200, vec![("Transfer-Encoding", "chunked")]),
    );
    for conn in p.conns {
        assert!(!conn.client_is_waiting_for_100_continue);
        assert!(!conn.they_are_waiting_for_100_continue);
    ...
    // Disabled by the client going ahead and sending stuff anyway
    p = ConnectionPair::new();
    p.send(Role::Client, Data::new(b"12345"));
    for conn in p.conns {
        assert!(!conn.client_is_waiting_for_100_continue);
        assert!(!conn.they_are_waiting_for_100_continue);
    }
